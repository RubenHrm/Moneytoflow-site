...
REWARD_PER_REF = float(os.getenv('REWARD_PER_REF','1700.0'))  # FCFA per buyer before commission multiplier
...
def commission_rate(buyers_count:int):
    # progressive tiers: base 20%, 30% at 50 buyers, 40% at 100 buyers
    if buyers_count >= 100:
        return 0.40
    if buyers_count >= 50:
        return 0.30
...
def dashboard():
    ...
    # amount in FCFA (buyers * per-ref * rate)
...
def confirm_purchase():
    if request.method == 'POST':
        ...
        uid = session['user_id']
        db = get_db()
        db.execute('INSERT INTO purchases (user_id, reference, validated, created_at) VALUES (?, ?, 0, ?)', (uid, reference, datetime.utcnow().isoformat()))
        db.commit()
...
def withdraw():
    ...
    if request.method == 'POST':
        ...
        if provider == 'Western Union':
            if not wu_name or not wu_country or len(mobile) < 6 or amount < WU_MIN:
                flash('Veuillez fournir toutes les informations requises pour Western Union (montant minimum: {})'.format(WU_MIN), 'danger')
                return redirect(url_for('withdraw'))
            db = get_db()
            db.execute('INSERT INTO withdrawals (user_id, provider, mobile_number, wu_fullname, wu_country, amount, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                       (uid, provider, mobile, wu_name, wu_country, amount, 'pending', datetime.utcnow().isoformat()))
            db.commit()
            flash("Demande de retrait WU envoyée à l'admin", 'success')
            return redirect(url_for('dashboard'))
        else:
            if len(mobile) < 6 or amount < MOBILE_MIN:
                flash('Numéro mobile invalide ou montant inférieur au minimum', 'danger')
                return redirect(url_for('withdraw'))
            db = get_db()
            db.execute('INSERT INTO withdrawals (user_id, provider, mobile_number, amount, status, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                       (uid, provider, mobile, amount, 'pending', datetime.utcnow().isoformat()))
            db.commit()
...
def admin_panel():
    ...
    withdrawals = query_db('SELECT w.id, w.user_id, w.provider, w.mobile_number, w.wu_fullname, w.wu_country, w.amount, w.status, w.created_at, u.username FROM withdrawals w JOIN users u ON w.user_id=u.id WHERE w.status!="validated"')
...
def support():
    if request.method == 'POST':
        ...
        db = get_db()
        db.execute('INSERT INTO tickets (user_id, email, subject, message, status, created_at) VALUES (?, ?, ?, ?, "open", ?)', (session.get('user_id'), email, subject, message, datetime.utcnow().isoformat()))
        db.commit()
